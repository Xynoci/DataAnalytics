...
with con:
	...
	for key in pop1940:
		insertStatement = """INSERT INTO popANDdensity VALUES('%s',%s,%s,%s,%s,%s,%s,%s,%s)""" % (key, pop1940[key], pop1950[key], pop1960[key], pop1970[key], pop1980[key], pop1990[key], pop2000[key], pop2010[key])
...
with con:
	...
	cur.execute("CREATE TABLE employment(neighborhood TEXT, Construction REAL, Manufacturing REAL, Retail REAL, TransportUtilities REAL, Information REAL, Finance REAL, Scientific REAL, EduHealthSoc REAL, ArtsRecreation REAL, PublicAdmin REAL, Other REAL)")
	for key in jobConstruction:
		insertStatement = """INSERT INTO employment VALUES('%s',%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""" % (key, jobConstruction[key], jobManufacturing[key], jobRetail[key], jobTransportUtilities[key], jobInformation[key], jobFinance[key], jobScientific[key], jobEduHealthSoc[key], jobArtsRecreation[key], jobPublicAdmin[key], jobOther[key])
		cur.execute(insertStatement)
	## NEEDED, if not, database does not update
	con.commit()
